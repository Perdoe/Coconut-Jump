...
pygame.display.set_caption("Coconut Jump")
clock = pygame.time.Clock()

# loading image-- image with transparent background

img = pygame.image.load("coconutJumper.png")
background = pygame.image.load("shorebackground.jpg")
...
def score(count, level):
    ...
    titleTextSurface, titleTextRectangle = makeTextObjs(score, smallText, levelText)
...
def blocks(x_block, y_block, blockWidth, blockHeight, gap, color):
    ...
    pygame.draw.rect(surface, color,
                     [x_block, y_block + blockHeight + gap, blockWidth, surfaceHeight - gap - blockHeight])

    # to get ring like shape at the tip of block(height =10 width =original +10)
    pygame.draw.rect(surface, pipeTop, [x_block - 5, blockHeight - 10, blockWidth + 10, 30])
    pygame.draw.rect(surface, pipeTop, [x_block - 5, y_block + blockHeight + gap, blockWidth + 10, 30])
...
def gameStart():
    # here we read high score from a log file and if a file does not exist we create a new one
    try:
        # reading highscore
        hscore = open("score.log", 'r')
        high_score = hscore.read()
    ...
    msgsurface("Press SPACEBAR to move")
...
def main():
    ...
    while not game_over:
        ...
        # drawing blocks
        blocks(x_block, y_block, blockWidth, blockHeight1, gap, pipeRoot)
...
BG = pygame.image.load("1280x720.png")

def get_font(size):  # Returns Press-Start-2P in the desired size
    return pygame.font.Font("white bread.ttf", size)
...
def play():
    while True:
        ...
        from coconut_jump import startGame
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
